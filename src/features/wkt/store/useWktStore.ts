...
interface WktStore {
  ...
  getGroupById: (id: string) => WktGroup | undefined;
  ...
  persist(
    (set, get) => ({
      ...
      removeEntry: (id: string) =>
        set((state) => {
          ...
          if (entry?.groupId) {
            ...
            // Remove empty groups
            nextGroups = nextGroups.filter((g) => g.entryIds.length > 0 || g.id !== entry.groupId);
          }
      ...
      toggleGroupExpanded: (id: string) =>
        set((state) => ({
          groups: state.groups.map((g) => (g.id === id ? { ...g, expanded: !g.expanded } : g)),
        })),
